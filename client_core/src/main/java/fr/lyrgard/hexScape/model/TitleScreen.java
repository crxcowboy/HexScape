...
public class TitleScreen implements Displayable {
	...
	public void populateNode() {
		...
		float y = -28f;
		TitleScreenSprite configLabel = new TitleScreenSprite(Type.SPRITE, "title/configLabel.png", x, y, sizeX, sizeY,
				null, null);
		x = 38;
		y = -28f;
		buttons.attachChild(new TitleScreenSprite(Type.CONFIG, "title/config.png", x, y, BUTTON_SIZE, BUTTON_SIZE,
				configLabel, node));
		
		//add quit button
		sizeX = 20;
		sizeY = 12;
		x = -40f - BUTTON_SIZE / 2 - sizeX / 2;
		y = -22.6f;
		TitleScreenSprite quitLabel = new TitleScreenSprite(Type.SPRITE, "title/quitLabel.png", x, y,
				sizeX, sizeY, null, null);
		x = -40f;
		y = -23.6f;
		buttons.attachChild(new TitleScreenSprite(Type.QUIT, "title/quit.png", x, y, BUTTON_SIZE,
				BUTTON_SIZE, quitLabel, node));
		...
		y = 21f;
		TitleScreenSprite multiplayerLabel = new TitleScreenSprite(Type.SPRITE, "title/multiplayerLabel.png", x, y,
				sizeX, sizeY, null, null);
		x = 20f;
		y = 19.4f;
		buttons.attachChild(new TitleScreenSprite(Type.MULTIPLAYER, "title/multiplayer.png", x, y, BUTTON_SIZE,
				BUTTON_SIZE, multiplayerLabel, node));

		sizeX = 30;
		sizeY = 8;
		x = -15.5f + BUTTON_SIZE / 2 + sizeX / 2;
		y = -26.5f;
		TitleScreenSprite soloLabel = new TitleScreenSprite(Type.SPRITE, "title/soloLabel.png", x, y, sizeX, sizeY,
				null, null);
		x = -15.5f;
		y = -23.6f;
		buttons.attachChild(
				new TitleScreenSprite(Type.SOLO, "title/solo.png", x, y, BUTTON_SIZE, BUTTON_SIZE, soloLabel, node));
		...
		backgroundGeom.setLocalTranslation(-100, -2, +75); // Need to Divide by two because the quad origin is bottom
															// left
